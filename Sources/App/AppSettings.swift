...
enum PrefKey {
    ...
    /// Chave antiga em UserDefaults — mantida só para migrar ao Keychain.
    static let claudeAPIKey = "claudeAPIKey"
    static let clipboardSkipConcealed = "clipboardSkipConcealed"
    static let clipboardSkipTransient = "clipboardSkipTransient"
    static let clipboardSkipSecretLike = "clipboardSkipSecretLike"
    static let clipboardRetentionHours = "clipboardRetentionHours"
...
enum Prefs {
    ...
    /// Vive no Keychain do macOS; ver APIKeyVault.
    static var claudeAPIKey: String? {
        APIKeyVault.key
    }

    /// Pula capturas marcadas como confidenciais por gerenciadores de
    /// senhas (org.nspasteboard.ConcealedType).
    static var clipboardSkipConcealed: Bool {
        UserDefaults.standard.object(forKey: PrefKey.clipboardSkipConcealed) as? Bool ?? true
    }

    /// Pula conteúdo transitório/gerado automaticamente
    /// (org.nspasteboard.TransientType / AutoGeneratedType).
    static var clipboardSkipTransient: Bool {
        UserDefaults.standard.object(forKey: PrefKey.clipboardSkipTransient) as? Bool ?? true
    }

    /// Detecção local de textos com cara de senha/chave/token.
    static var clipboardSkipSecretLike: Bool {
        UserDefaults.standard.object(forKey: PrefKey.clipboardSkipSecretLike) as? Bool ?? true
    }

    /// Expiração automática do histórico do clipboard, em horas
    /// (0 = manter para sempre). Padrão: 7 dias.
    static var clipboardRetentionHours: Double {
        guard let value = UserDefaults.standard.object(forKey: PrefKey.clipboardRetentionHours) as? Double
        else { return 168 }
        return max(0, value)
    }
